...
* Sau khi xác thực thành công, Istio tự động trích xuất các claims và tiêm (inject) các header sau vào các request gửi tới các service nội bộ:
  ...
  * `user-role`: Vai trò của người dùng (`CLIENT`, `COMPANION`, `ADMIN`)
  * `user-status`: Trạng thái tài khoản (`ACTIVE`, `LOCKED`)
...
* **Query Parameters:**
  * `status`: Lọc theo trạng thái cuộc hẹn (`PENDING`, `ACCEPTED`, `COMPLETED`, `CANCELLED`, `DISPUTED`, `RESOLVED`).
...
* **Request Body:**
  ...
  {
    "bookingId": "bk_cinema_888",
    "reason": "Bận lịch cá nhân đột xuất"
  }
  ```
* **Phản hồi thành công (200 OK):**
  ```json
...
* **Query Parameters:**
  * `status`: Trạng thái khiếu nại (`OPEN`, `RESOLVING`, `REFUNDED`, `PAID_OUT`, `REJECTED`).
  * `page` (mặc định 1) / `pageSize` (mặc định 10).
* **Phản hồi thành công (200 OK):**
  ```json
  {
...
### 2.7. REALTIME NOTIFICATIONS (SSE & REST INBOX)
Dịch vụ cung cấp stream thông báo thời gian thực thông qua Server-Sent Events (SSE) và các API quản lý hộp thư thông báo (Inbox).
...
* **Authentication:** Bearer JWT
* **Mô tả:** Tải danh sách thông báo phân trang cursor-based của người dùng hiện tại (Inbox).
* **Query Parameters:**
  * `cursor` (Tùy chọn): Chuỗi cursor lấy từ lần gọi trước để lấy trang tiếp theo (mã hóa Base64).
  * `limit` (Mặc định 20, tối đa 100): Số lượng bản ghi tối đa trả về trên một trang.
  * `unreadOnly` (Mặc định false): Chỉ lấy thông báo chưa đọc.
* **Phản hồi thành công (200 OK):**
  ```json
  {
    "data": [
      {
        ...
        "payload": {
          ...
          "body": "Bạn có yêu cầu hẹn hò xem phim mới từ Kazuya.",
          "bookingId": "bk_cinema_888"
        },
        "status": "COMPLETED",
        "readAt": null,
  ...
  ```
  *Lưu ý về các Enum & Trường dữ liệu:*
  * `type` (Loại thông báo): `TRANSACTIONAL`, `INTERACTION`, `PROMOTIONAL`.
  * `priority` (Độ ưu tiên): `LOW`, `MEDIUM`, `HIGH`.
  * `status` (Trạng thái truyền tin): `PENDING`, `PROCESSING`, `COMPLETED`, `FAILED`. Trạng thái đọc/chưa đọc của thông báo được kiểm tra qua trường `readAt` (`null` có nghĩa là chưa đọc, khác `null` trả về thời điểm đã đọc).

#### `PATCH /v1/notifications/{id}/read`
* **Authentication:** Bearer JWT
* **Mô tả:** Đánh dấu một thông báo cụ thể là đã đọc.
* **Path Parameters:**
  * `id` (Bắt buộc): UUID của thông báo cần đánh dấu.
* **Phản hồi thành công (204 No Content):** Không có response body.

#### `PATCH /v1/notifications/read-all`
* **Authentication:** Bearer JWT
* **Mô tả:** Đánh dấu toàn bộ thông báo chưa đọc của người dùng hiện tại là đã đọc.
* **Phản hồi thành công (200 OK):**
  ```json
  {
    "affectedRows": 5
