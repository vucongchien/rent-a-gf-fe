...
*   **Mô tả:** Khởi tạo luồng đăng nhập Google OAuth2. Trả về URL để redirect người dùng sang trang đăng nhập của Google cùng PKCE state.
...
*   **Mô tả:** Client gửi yêu cầu nâng cấp tài khoản lên thành Companion.
*   **Request Body:**
    ```json
    {
...
*   **Mô tả:** Admin lấy danh sách các yêu cầu nâng cấp lên Companion.
*   **Response Body (Success):**
    ```json
    {
      "data": [
...
*   **Query Parameters:**
    ...
    *   `minPrice` / `maxPrice`: Khoảng giá kịch bản (Kano-Coin)
...
*   **Response Body (Success):**
    ```json
    {
      "companionId": "cmp_chizuru_123",
      "displayName": "Chizuru Ichinose",
      "introText": "Hãy để tôi đóng vai một người bạn gái...",
      "avatarUrl": "https://storage.rent-a-gf.com/avatars/chizuru.png",
      "albumUrls": [],
      "voiceIntroUrl": "https://storage.rent-a-gf.com/voice/chizuru.mp3",
      "availableCities": ["HCM", "Danang"],
...
*   **Query Parameters:**
    *   `view`: Lọc theo view (`pending`, `upcoming`, `history`)
    *   `pageSize` / `pageToken`: Phân trang và giới hạn số lượng
*   **Response Body (Success):**
    ```json
    {
      "bookings": [
...
*   **Mô tả:** Lấy thông tin chi tiết trạng thái của cuộc hẹn.
*   **Response Body (Success):**
    ```json
    {
      "bookingId": "bk_cinema_888",
...
*   **Request Body:**
    ```json
    {
      "reason": "Tôi bận việc đột xuất vào giờ này."
    }
    ```
*   **Response Body (Success):**
    ```json
...
*   **Request Body:**
    ```json
    {
      "reason": "CANCELLATION_REASON_CLIENT_EARLY"
    }
    ```
    *Các mã lý do CancellationReason hợp lệ: CANCELLATION_REASON_CLIENT_EARLY, CANCELLATION_REASON_CLIENT_LATE, CANCELLATION_REASON_COMPANION_EARLY, CANCELLATION_REASON_COMPANION_LATE.*
*   **Response Body (Success):**
    ```json
    {
      "bookingId": "bk_cinema_888",
...
*   **Authentication:** None (Thường chạy tự động bởi hệ thống hoặc Client)
...
#### [GET] `/notifications`
*   **Authentication:** Access Token (Bearer)
*   **Mô tả:** Tải danh sách thông báo phân trang cursor-based của người dùng.
*   **Query Parameters:**
    *   `cursor` (string, optional): Chuỗi Cursor lấy từ lần gọi trước để lấy trang tiếp theo. Để trống nếu tải lần đầu.
    *   `limit` (int, optional): Số lượng bản ghi tối đa (mặc định 20, tối đa 100).
    *   `unreadOnly` (boolean, optional): Lọc các tin chưa đọc (mặc định false).
*   **Response Body (Success):**
    ```json
    {
      "data": [
...
*   **Authentication:** Access Token (Bearer)
*   **Mô tả:** Đánh dấu toàn bộ thông báo chưa đọc của người dùng là đã đọc.
*   **Response Body (Success):**
    ```json
    {
      "affectedRows": 5
